...
/**
 * MovieController is responsible for managing Movie in the system. It provides a RESTful API that
 * supports standard CRUD operations. Each operation is mapped to the conventional HTTP methods in
 * REST: POST to create new movie, Get for fetching all the movies, PUT to update specific movie by
 * id, and DELETE for delete one by id. Created by Tal Cohen.
 */
@RestController
...
public class MovieController {
  ...
  public ResponseEntity<?> createNewMovie(@Valid @RequestBody MovieRequest movie) {
    return movieService
        ...
        .map(
            saved -> ResponseEntity.created(URI.create("/api/movies/" + saved.getId())).body(saved))
  ...
  public ResponseEntity<Movie> updateMovie(
      @PathVariable Long id, @RequestBody MovieRequest movieDetails) {
    return movieService
